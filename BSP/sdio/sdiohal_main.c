...
static int sdiohal_dt_set_addr(unsigned int addr)
{
	...
	//pr_info("dt readl enter sdiohal_dt_set_addr\n");
	sdio_claim_host(p_data->sdio_func[FUNC_0]);
...
exit:
	...
	if (err != 0)
	{
		pr_err("dt readl fail sdiohal_dt_set_addr\n");	
	}
...
int sdiohal_readl_no_wake(unsigned int system_addr, void *buf)
{
	struct sdiohal_data_t *p_data = sdiohal_get_data();
	int ret = 0;
...
int sdiohal_readl(unsigned int system_addr, void *buf)
{
	...
	if (IS_BYPASS_WAKE(system_addr))
		return sdiohal_readl_no_wake(system_addr, buf);
	...
	sdiohal_op_enter();
	pr_debug("sdiohal_readl enter\n");
	ret = sdiohal_dt_set_addr(system_addr);
	if (ret != 0) {
		sdiohal_op_leave();
		pr_info("sdiohal_readl ret:%d\n", ret);
...
static int sdiohal_parse_dt(void)
{
	...
	{
		// ADMA RX blk_size 840
		p_data->adma_rx_enable = true;
		p_data->blk_size = false;
	}
	else
	{
		// SDMA RX blk_size 512
		p_data->blk_size = true;
	}
	...
	ret = of_property_read_string(np, "sdio-irq-type",
				      (const char **)&sdio_irq_type);
	/* irq type */
	if (!ret)
	{
		if(!strcmp(sdio_irq_type, "data-irq"))
		{
			p_data->irq_type = SDIOHAL_RX_INBAND_IRQ;			
		}
		else if(!strcmp(sdio_irq_type, "rx-polling"))
		{
	...
	if(p_data->irq_type != SDIOHAL_RX_INBAND_IRQ)
	{
		...
		if (!gpio_is_valid(p_data->gpio_num)) {
			pr_err("can not get sdio int gpio%d\n", p_data->gpio_num);
		}
	}
	...
	const struct sdio_device_id *id)
{
...
#ifdef CONFIG_THIRD_PARTY_BOARD 
	pr_info("%s: func->class=%x, vendor=0x%04x, device=0x%04x, "
	...
	if(p_data->irq_type != SDIOHAL_RX_INBAND_IRQ)
	{
		...
		if(p_data->irq_num == 0 && gpio_is_valid(p_data->gpio_num)){
...
#if  defined(CONFIG_UNISOC_BOARD) || defined(CONFIG_NXP_BOARD)
		/* In Unisoc Board(only 8581a_5h10), use notify to trigger scan finish here, 
		   will lead to kernel crash, so we call external function symbol instead.
		*/
		marlin_scan_finish();
#else
		scan_card_notify();
#endif
#ifdef CONFIG_THIRD_PARTY_BOARD 
	pr_info("%s: sdio_dev_host, max_req_size=0x%x, max_blk_count=0x%x, max_blk_size=0x%x\n",
...
int quectel_platform_rescan_card(void)
{
    int count = 100;

    struct mmc_host *wlan_mmc = NULL;
    struct sdhci_host *host;
    struct device *dev = bus_find_device_by_name(&platform_bus_type , NULL , "7824900.sdhci");
    struct platform_device * sdio1_device = to_platform_device(dev);

    pr_info("%s dev %x , sdio1 %x \n", __func__ , dev , sdio1_device);
    ...
    if(wlan_mmc) {
        wlan_mmc->rescan_disable = 0;
        wlan_mmc->rescan_entered = 0;
        mmc_detect_change(wlan_mmc , 0);
    } else {
        pr_info("%s wlan_mmc is null , craddetect fail \n" , __func__);
    }

    do {
        if (wlan_mmc->card && mmc_card_present(wlan_mmc->card)) {
            pr_info("%s %d sdio card detected" , __func__, __LINE__);
            return 1;
        }
        msleep(10);
    } while (count--);

    pr_info("%s %d sdio card no exist.\n" , __func__, __LINE__);
    return 0;
}
#endif
...
#endif
#endif
	if (WCN_CARD_EXIST(&sdiohal_data->xmit_cnt)) {
		pr_info("Already exist card!\n");
		sdiohal_remove_card(wcn_dev);
	}
#ifdef CONFIG_AML_BOARD
