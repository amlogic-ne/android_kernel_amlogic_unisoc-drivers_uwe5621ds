...
static enum wcn_hard_intf_type wcn_hw_type;
...
static ssize_t debugsuspend_store(struct device* dev,
    ...
    const char* buf, size_t count)
{
    ...
    if (memcmp("AT+SUSPEND=", buf, strlen("AT+SUSPEND=")) == 0 && count >= strlen("AT+SUSPEND=") + 1) {
        unsigned char is_resume = 0;
		tbuf += strlen("AT+SUSPEND=");
		STREAM_TO_UINT8(is_resume, tbuf);
        is_resume = is_resume - '0';
		pr_info("%s AT+SUSPEND=%d\n", __func__, is_resume);
...
static int  mtty_probe(struct platform_device *pdev)
{
   ...
   /*if (pdev->dev.of_node && !pdata) {
    ...
    pr_info("mtty_probe get hw type:%d\n", wcn_hw_type);
