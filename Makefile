...
DRIVER_KOS := BSP/uwe5621_bsp_sdio.ko WIFI/sprdwl_ng.ko BT/sprdbt_tty.ko
...
modules modules_install clean:
	...
	$(MAKE) -C $(KERNEL_SRC) M=$(M)/BT CFG_AML_WIFI_DEVICE_UWE5621=y UNISOC_WCN_CHIP_ID=uwe5621 COUSTOM_PLATFORM=aml UNISOC_WCN_HW_TYPE=sdio $(@)
	rm -f $(out_dir)/$(M)/Module.symvers
	for symvers in $(out_dir)/$(M)/BSP/Module.symvers $(out_dir)/$(M)/WIFI/Module.symvers $(out_dir)/$(M)/BT/Module.symvers; do \
		if [ -f $$symvers ]; then \
	...
	for morder in $(out_dir)/$(M)/BSP/modules.order $(out_dir)/$(M)/WIFI/modules.order $(out_dir)/$(M)/BT/modules.order; do \
		if [ -f $$morder ]; then \
